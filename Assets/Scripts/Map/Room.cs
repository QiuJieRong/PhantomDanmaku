...
public class Room
{
    ...
    public RoomInfo Info
    {
        get
        {
            return roomPrefab.GetComponent<RoomInfo>();
        }
    }
    ...
    public void DrawGround()
    {
        ...
        for (int i = 0; i < Info.Width; i++)
        {
            ...
            {
                Vector3Int drawPos = (Vector3Int)(CenterCoord + new Vector2Int(i, j) - new Vector2Int(Info.Width / 2, Info.Height / 2));
    ...
    public void DrawWall()
    {
        ...
        {
            Vector3Int drawPos = (Vector3Int)(CenterCoord + new Vector2Int(i, Info.Height) - new Vector2Int(Info.Width / 2, Info.Height / 2));
            tilemap_wall.SetTile(drawPos, Info.tileBase_wall);
        }
        //底部
        for (int i = -1; i <= Info.Width; i++)
        {
            Vector3Int drawPos = (Vector3Int)(CenterCoord + new Vector2Int(i, -1) - new Vector2Int(Info.Width / 2, Info.Height / 2));
            tilemap_wall.SetTile(drawPos, Info.tileBase_wall);
        }
        //右侧
        for (int i = -1; i <= Info.Height; i++)
        {
            Vector3Int drawPos = (Vector3Int)(CenterCoord + new Vector2Int(Info.Width, i) - new Vector2Int(Info.Width / 2, Info.Height / 2));
            tilemap_wall.SetTile(drawPos, Info.tileBase_wall);
        }
        //左侧
        for (int i = -1; i <= Info.Height; i++)
        {
            Vector3Int drawPos = (Vector3Int)(CenterCoord + new Vector2Int(-1, i) - new Vector2Int(Info.Width / 2, Info.Height / 2));
            tilemap_wall.SetTile(drawPos, Info.tileBase_wall);
        }
    }
        ...
        if (roomMap.Contains(gridCoord + new Vector2Int(0, 1)))
        {
            ...
            for (int i = 0; i <= roadLength; i++)
            {
                ...
                for (int j = 0; j <= roadWidth / 2; j++)
                {
                    ...
                    {
                        tilemap_ground.SetTile((Vector3Int)curpoint + new Vector3Int(-j, 0, 0), Info.tileBase_ground);
                        tilemap_ground.SetTile((Vector3Int)curpoint + new Vector3Int(j, 0, 0), Info.tileBase_ground);
                    }
                }
                //绘制道路的墙壁
                tilemap_wall.SetTile((Vector3Int)curpoint + new Vector3Int(roadWidth / 2 + 1, 0, 0), Info.tileBase_wall);
                tilemap_wall.SetTile((Vector3Int)curpoint + new Vector3Int(-roadWidth / 2 - 1, 0, 0), Info.tileBase_wall);
        ...
        if (roomMap.Contains(gridCoord + new Vector2Int(1, 0)))
        {
            ...
            for (int i = 0; i <= roadLength; i++)
            {
                for (int j = 0; j <= roadWidth / 2; j++)
                {
                    ...
                    {
                        tilemap_ground.SetTile((Vector3Int)curpoint + new Vector3Int(0, -j, 0), Info.tileBase_ground);
                        tilemap_ground.SetTile((Vector3Int)curpoint + new Vector3Int(0, j, 0), Info.tileBase_ground);
                    }
                }
                //绘制道路两边的墙壁
                tilemap_wall.SetTile((Vector3Int)curpoint + new Vector3Int(0, roadWidth / 2 + 1, 0), Info.tileBase_wall);
                tilemap_wall.SetTile((Vector3Int)curpoint + new Vector3Int(0, -roadWidth / 2 - 1, 0), Info.tileBase_wall);
